...
const TaskItem: React.FC<TaskItemProps> = ({ 
  ...
  return (
    ...
    >
      {dropPosition === 'top' && (
        <div className="absolute top-0 left-0 right-0 h-1 bg-blue-500 rounded-full -mt-0.5 z-10" />
      )}
      <div
        onClick={onSelect}
        draggable
      ...
      >
        <div className="flex justify-between items-start gap-2">
          <p
            className={`font-medium pr-2 flex-grow ${
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="relative" onClick={e => e.stopPropagation()}>
              ...
              <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">
                <svg
                  className="w-4 h-4 fill-current text-white opacity-70"
                  viewBox="0 0 20 20"
                >
                  <path d="M5.516 7.548c.436-.446 1.144-.446 1.58 0L10 10.43l2.904-2.882c.436-.446 1.144-.446 1.58 0 .436.446.436 1.17 0 1.615l-3.694 3.664c-.436.446-1.144.446-1.58 0L5.516 9.163c-.436-.446-.436-1.17 0-1.615z"></path>
                </svg>
              </div>
            </div>
          </div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          {projectName ? (
            <p
              className="text-xs text-slate-600 font-medium bg-slate-200 px-2 py-0.5 rounded-full truncate"
              title={projectName}
            >
              {projectName}
            </p>
          ...
          <div className="flex flex-col items-end">
            {task.status !== TaskStatus.Done && incompleteFollowUpsCount > 0 && (
              <p className="text-xs text-amber-700 font-medium">
                Pending on: {incompleteFollowUpsCount} follow-up
                {incompleteFollowUpsCount > 1 ? 's' : ''}
              </p>
            )}
          </div>
        </div>
      ...
      {dropPosition === 'bottom' && (
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-blue-500 rounded-full -mb-0.5 z-10" />
      )}
    </div>
  );
};
...
export const TaskList: React.FC<TaskListProps> = ({ 
  ...
  const handleDragOver = (e: React.DragEvent, targetTaskId: string) => {
    ...
    const newPosition = isOverTopHalf ? 'top' : 'bottom';
    if (dropTarget?.id !== targetTaskId || dropTarget?.position !== newPosition) {
      setDropTarget({
        id: targetTaskId,
  ...
  // --- Today's Focus layout with hard height for Top Priorities + scrollable Full List ---
  if (title === "Today's Focus") {
    ...
    return (
      <div className="h-full bg-slate-100 p-4 flex flex-col overflow-hidden">
        {/* Whole column is constrained and uses its height fully */}
        <div className="flex flex-col gap-6 h-full overflow-hidden">
          {/* Top Priorities: fixed height, scroll only inside this box */}
          <section className="shrink-0">
            ...
            <div
              ...
              className={`p-2 space-y-3 bg-slate-200/50 rounded-lg border-2 border-dashed transition-colors
                ${isPriorityZoneDragOver ? 'border-blue-500 bg-blue-50' : 'border-slate-300'}
                h-[18rem] overflow-y-auto
              `}
            >
          ...
          {/* Full List: uses remaining vertical space and scrolls in its own area */}
          <section className="flex-1 min-h-0 flex flex-col">
            ...
            <div 
              ...
              className={`space-y-3 overflow-y-auto flex-1 p-2 rounded-lg border-2 border-dashed transition-colors ${
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          {isProjectPanelCollapsed && (
            <button
              onClick={onExpand}
              className="p-1 rounded-full hover:bg-slate-200 -ml-1 mr-2"
        ...
        {title === 'Tasks' && onToggleHideCompleted && (
      ...
      <div className="flex-grow overflow-y-auto space-y-3 pr-1">
        {tasks.length > 0 ? (
          ...
          <div className="text-center text-slate-500 pt-16">
            ...
            <p>{canAddTask ? 'Add your first task above!' : 'Great job!'}</p>
          </div>
        )}
      </div>
    </div>
